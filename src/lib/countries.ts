/**
 * Every ISO-3166 country with its E.164 dial code.
 *
 * Only the code→dial pairs are hand-maintained; the display names come from
 * `Intl.DisplayNames`, so they stay correct (and spelled the way the platform
 * spells them) without a second list to keep in sync.
 */

// "ISO:dial" pairs, compact on purpose — this list is data, not code.
const DIAL_CODES =
  ...
  "CA:1,CC:61,CD:243,CF:236,CG:242,CH:41,CI:225,CK:682,CL:56,CM:237,CN:86,CO:57,CR:506,CU:53,CV:238," +
  ...
  "MA:212,MC:377,MD:373,ME:382,MF:590,MG:261,MH:692,MK:389,ML:223,MM:95,MN:976,MO:853,MP:1,MQ:596," +
  "MR:222,MS:1,MT:356,MU:230,MV:960,MW:265,MX:52,MY:60,MZ:258," +
  ...
  "SA:966,SB:677,SC:248,SD:249,SE:46,SG:65,SH:290,SI:386,SJ:47,SK:421,SL:232,SM:378,SN:221,SO:252," +
...
export type Country = {
  /** ISO-3166 alpha-2, e.g. "IN" — the single source of truth in the form. */
  code: string;
  /** Display name, e.g. "India". */
  name: string;
  /** E.164 dial code WITHOUT the plus, e.g. "91". */
  dial: string;
...
export const COUNTRIES: Country[] = DIAL_CODES.split(",")
  .map((pair) => {
    const [code, dial] = pair.split(":");
    return {
      code,
      // Fall back to the raw code on platforms without Intl.DisplayNames.
...
/** Dial-code order for the phone prefix picker: numeric, then by name. */
export const COUNTRIES_BY_DIAL: Country[] = [...COUNTRIES].sort(
  (a, b) => Number(a.dial) - Number(b.dial) || a.name.localeCompare(b.name)
);
